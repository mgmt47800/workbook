...
You've established the directory where your virtual environment will be stored (`C:\conda_envs`) and you've installed Miniforge, which helps you create those environments.

Now you will configure **Conda** to use your external directory, and optionally install **Mamba** for faster package management. Here's an explanation:
...
## Conda-forge: The Free Package Repository
Because you installed **Miniforge**, your system is already configured to use **conda-forge** as the default package source. This is a community-maintained repository with thousands of free, open-source packages for data science and analytics. You don't need to worry about licensing restrictions or commercial repositories.

## Mamba: The High-Speed Engine (Optional)
...
2. **Verify Path Priority and Configuration**
   * In the Miniforge prompt, type `conda info` and press {kbd}`Enter`
   * Scroll to the **envs directories** section in the output
   * Confirm that `C:\conda_envs` appears as the first (top) entry in the list
   * Scroll to the **channel URLs** section and confirm you see `conda-forge` as the default channel
...
3. **Install Mamba (Optional but Recommended)**
   * In the Miniforge prompt, copy the following command:
     ```
     conda install mamba -y
     ```
   * Paste it and press {kbd}`Enter`
   * Wait for the installation to complete
   * After installation, you can use `mamba` instead of `conda` for faster package installations
