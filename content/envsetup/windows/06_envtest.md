...
label: envsetup-windows-env-test
---
...
1. **Open the Project Subdirectory in VS Code**
   ...
   * Double-click the `getting_started` directory and click the "Select Folder" button
...
2. **Open VS Code's Built-In Terminal**
   ...
   * Identify the Shell: Look at the terminal dropdown menu in the top-right area of the terminal panel. It should display **bash** or **Git Bash**.
      * If the terminal shows **powershell** or **cmd** instead, click the **down arrow** next to the {kbd}`+` icon in the terminal window and select **Git Bash**.
   * Verify via Command: Type the following command into the terminal and press {kbd}`Enter`:
    ```bash
    echo $SHELL
    ```
   :::{note}
   The output should show `/usr/bin/bash` or something similar containing "bash". 
   :::
...
3. **Create the Virtual Environment**
   * In the Terminal _within VS Code_, type the following command and press {kbd}`Enter`:
   ...
   ```
      ...
      * You may see Poetry creating a virtual environment and downloading packages - this is normal and may take a minute or two

4. **Execute and Verify**
   * Look at the top-right corner of the notebook editor; if it says "Select Kernel," click it and choose **Python Environments...**
   * Review the list of environments. You should see one that starts with `getting_started...` or shows `.venv` - Select that environment
